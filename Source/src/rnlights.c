...
void CalcAverageCol(LIGHTS_STATE_STRUCT *lightState) {
...
#else
                    fAmbientVecLength = (float) sqrt(
                            3.0f * (lightState->ambient_colour[0] * lightState->ambient_colour[0]));
#endif
                } else {
    ...
    for (curEntry = 0; curEntry < lightState->num_lights; curEntry++) {
        ...
        if (lightState->light_entries[curEntry].light_flags & light_on) {
            ...
            lightState->light_entries[curEntry].smooth_intensity =
                    fVecLength * SummedBrightness[nSlot] * fRescale[nSlot];
        }
    }
