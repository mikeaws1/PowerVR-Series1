...
static void Intersect(PTRANSTRI_STRUCT *ppsTriNodes, PTRANSTRI_STRUCT *ppsLeftStack, PTRANSTRI_STRUCT *ppsRightStack,
                      unsigned bIsAscending, unsigned uNumTris, unsigned uLevel);
...
*/
static INLINE sgl_uint32
LinesIntersect(float *fLineA, float *fLineB, float fLineAVert1[2], float fLineAVert2[2], float fLineBVert1[2],
               float fLineBVert2[2], float fPoint[2], sgl_uint32 *puAInsides) {
    sgl_uint32 uGarbage;
    float fCross;
...
static sgl_uint32 Overlaps(PTRANSTRI_STRUCT psTriA, PTRANSTRI_STRUCT psTriB, float fPoint[2]) {
    ...
    if (LinesIntersect(&psTriA->fAdjoint[0][0], &psTriB->fAdjoint[0][0], psTriA->fVerts[1], psTriA->fVerts[2],
                       psTriB->fVerts[1], psTriB->fVerts[2], fPoint, &uBInsideA))
        return 1;
    if (LinesIntersect(&psTriA->fAdjoint[0][0], &psTriB->fAdjoint[0][1], psTriA->fVerts[1], psTriA->fVerts[2],
                       psTriB->fVerts[2], psTriB->fVerts[0], fPoint, &uBInsideA))
        return 1;
    if (LinesIntersect(&psTriA->fAdjoint[0][0], &psTriB->fAdjoint[0][2], psTriA->fVerts[1], psTriA->fVerts[2],
                       psTriB->fVerts[0], psTriB->fVerts[1], fPoint, &uBInsideA))
        return 1;

    if (LinesIntersect(&psTriA->fAdjoint[0][1], &psTriB->fAdjoint[0][0], psTriA->fVerts[2], psTriA->fVerts[0],
                       psTriB->fVerts[2], psTriB->fVerts[1], fPoint, &uBInsideA))
        return 1;
    if (LinesIntersect(&psTriA->fAdjoint[0][1], &psTriB->fAdjoint[0][1], psTriA->fVerts[2], psTriA->fVerts[0],
                       psTriB->fVerts[2], psTriB->fVerts[0], fPoint, &uBInsideA))
        return 1;
    if (LinesIntersect(&psTriA->fAdjoint[0][1], &psTriB->fAdjoint[0][2], psTriA->fVerts[2], psTriA->fVerts[0],
                       psTriB->fVerts[0], psTriB->fVerts[1], fPoint, &uBInsideA))
        return 1;

    if (LinesIntersect(&psTriA->fAdjoint[0][2], &psTriB->fAdjoint[0][0], psTriA->fVerts[0], psTriA->fVerts[1],
                       psTriB->fVerts[1], psTriB->fVerts[2], fPoint, &uBInsideA))
        return 1;
    if (LinesIntersect(&psTriA->fAdjoint[0][2], &psTriB->fAdjoint[0][1], psTriA->fVerts[0], psTriA->fVerts[1],
                       psTriB->fVerts[0], psTriB->fVerts[2], fPoint, &uBInsideA))
        return 1;
    if (LinesIntersect(&psTriA->fAdjoint[0][2], &psTriB->fAdjoint[0][2], psTriA->fVerts[0], psTriA->fVerts[1],
                       psTriB->fVerts[0], psTriB->fVerts[1], fPoint, &uBInsideA))
        return 1;
    ...
    */
    ...
    for (i = 0; i < 3; i++) {
        if (Sgn(psTriB->fAdjoint[0][i] * fPoint[0] + psTriB->fAdjoint[1][i] * fPoint[1] + psTriB->fAdjoint[2][i]) ==
            -1) {
            return 0;
        }
    }
...
*/
static INLINE void
Partition(PTRANSTRI_STRUCT *ppsTriNodes, PTRANSTRI_STRUCT **pppsLeftStack, PTRANSTRI_STRUCT **pppsRightStack,
          unsigned bIsAscending, unsigned uNumTris, float fLine[3], unsigned *puNumLeft, unsigned *puNumShared,
          unsigned *puNumRight) {
    ...
    for (i = 0; i < uNumItris; i++) {
        for (j = i - 1; j >= 0; j--) {
            ...
            if (psLocalNodes[j].psMarkedBy != &psLocalNodes[i]) {
                if (
                        (
                                (Hash[h].p[0] == ppsTriNodes[i] && Hash[h].p[1] == ppsTriNodes[j] &&
...
*/
static INLINE void
ChooseSplitLine(PTRANSTRI_STRUCT *ppsTriNodes, unsigned uNumTris, float fLine[3], unsigned bIsAscending) {
...
static void Intersect(PTRANSTRI_STRUCT *ppsTriNodes, PTRANSTRI_STRUCT *ppsLeftStack, PTRANSTRI_STRUCT *ppsRightStack,
    ...
    */
    Partition(ppsTriNodes, &ppsLeftStack, &ppsRightStack, bIsAscending, uNumTris, fLine, &uNumLeft, &uNumShared,
              &uNumRight);
