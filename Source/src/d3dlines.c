 ...
 *                D3D Line rendering mode. Taken from dlines.c .
 ...
 * Modifications:
 * $Log: d3dlines.c,v $
 * Revision 1.5  1998/01/26  18:13:15  erf
 * Added code to implement Z >= mode.
 *
 * Revision 1.4  1997/11/17  12:49:10  erf
 * Changed TSP increment of PackHighShad in ShadowFuncs from 6 to 4.
 *
 * Revision 1.3  1997/11/10  14:37:06  gdc
 * added depth bias
 *
 * Revision 1.2  1997/10/27  10:37:31  erf
 * Need to check for Z > 1. Just incase the app is not correct.
 * D3D specifies 0 <= Z < 1.
 *
 * Revision 1.1  1997/10/27  10:23:30  erf
 * Initial revision
 *
 *
 *****************************************************************************/
...
#else
            pLine->fZ1 = (gfInvertZ - pVert1->fZ) * fMinInvZ;
            pLine->fZ2 = (gfInvertZ - pVert2->fZ) * fMinInvZ;
...
void DirectLines(PSGLCONTEXT pContext,
    ...
    */
    ...
    if (gpLineDC->Context.u32Flags & SGLTT_TEXTURE) {
         ...
         */
        TPS.TexAddress = (sgl_uint32) gpLineDC->Context.nTextureName;
