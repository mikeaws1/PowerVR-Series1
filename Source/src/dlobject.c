...
static int nClosedRevolve(int nNoPoints, float pt[][3], sgl_vector a, sgl_vector b,
    ...
    if (EdgePerpToAxis(pt[i], pt[i - 1])) {
        ...
        vAddPlane(pt[nNoPoints - 1], lattice(nNoPoints - 2, 1), pt[nNoPoints - 2], a, b, bSmoothShade, 0, bTexture, 0,
                  1, 0);
	...
	*/
    ...
    for (i = 0; i < nNoPoints; i++) /* for each edge */
    {
		...
		*/
        ...
        if (nSetPrevInside) {    /* previous edge is dependent on orientation of this edge */
            nSetPrevInside = FALSE;
