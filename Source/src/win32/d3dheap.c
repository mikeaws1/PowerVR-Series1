 ...
 * License		: MIT
 *
 * Description    :	WIN32 heap services. Non shared memory alocation for
 *					PVRMID.dlls. Creates extra large heap for DAG sorting.
 *                   
 * Program Type   :	WIN32 DLL
 *
 * RCS info:
 *
 * $Date: 1997/09/19 13:43:32 $
 * $Revision: 1.1 $
 * $Locker:  $
 * $Log: d3dheap.c,v $
 * Revision 1.1  1997/09/19  13:43:32  erf
 * Initial revision
 *
 *
 *;--
...
int MemInit(void) {
    if (g_hDDHAL_DriverHeap == NULL) {
        /* Create non-sharable heap.
         */
        g_hDDHAL_DriverHeap = HeapCreate(0, 0x500000, 0);
        if (g_hDDHAL_DriverHeap == NULL) {
            return FALSE;
        }
    }
