...
{                           
	...
	num_sign=(((0x00000001L<<(MANT_BITS-1)) & number)!=0);
 ...
 * Inputs         : i,u,shad,pvis,luid,pper,muxsel - setting for the commands data
 * Outputs        :  
 * Input/Output	  : 
...
cell_control	wideCom(int i,int u,int shad,BOOL pvis,BOOL luid,BOOL pper,BOOL muxsel)
{
...
cell_control ExpandInstruction(int instr,BOOL do_nop,BOOL sec_object)
{
	...
	else
	{
		...
		switch(instr)	
		{
			...
			case begin_trans:
				wide=wideCom(load_i,load_u_closer,test_shad_nop,TRUE,TRUE,FALSE,TRUE);
...
int	SurfProcess(cell_state *cell,cell_control	instruction,long	C,unsigned long index,BOOL *shadow,long *U_plane_depth)
{
	...
	*/
	...
	if (instruction.load_u_id) /* this is only true for firstplane of translucency pass */ 
		cell->U_id=0; 
