 ...
 * Modifications:-
 * $Log: hwregs.c,v $
 * Revision 1.19  1997/04/10  19:57:52  mjg
 * Removed HWScheduleScreenSwap
 *
 * Revision 1.18  1996/12/11  01:24:34  jop
 * Removed ../'s from includes
 *
 * Revision 1.17  1996/10/09  17:45:34  jop
 * Region data offset added
 *
 * Revision 1.16  1995/12/21  16:04:56  sjf
 * added a #define
 * /
 *
...
{
	DPF((DBG_MESSAGE, "Setting sabre Region index to:0x%lx",RegionBase + RegionOffset));
	ParamStartAddrReg = RegionBase + RegionOffset;
}
	...
	*/
	...
	else if(!TexasBuffBInUse)
	{
		...
		*/
		...
		pTexasParamBuf->pParamStore = pTexasStore;
		pTexasParamBuf->StartIndex	= (TexasMemLimit / 2 + 2) & ~1L;
...
int	HWInitParamMem()
{
	...
	if (!Allocated)
	{
		...
		pTexasStore = (UINT32 *) calloc(TEXAS_MEM_SIZE,sizeof(UINT32));
		if ( pTexasStore ==NULL )
		{
			return err;
		}
		...
		TexasMemLimit = TEXAS_MEM_SIZE-1;
